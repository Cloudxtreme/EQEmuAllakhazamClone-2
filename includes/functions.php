...
)
{
    ...
    $mysql_rows_returned = $db->mysql_num_rows($mysql_reference_data);
    ...
    if ($mysql_rows_returned == 0) {
        ...
        for ($j = 1; $j <= $mysql_rows_returned; $j++) {
            $row = $db->mysql_fetch_array($mysql_reference_data);
...
{
    global $db, $spells_table, $spell_globals_table, $use_spell_globals;
    ...
    $result = db_mysql_query($query) or message_die('functions.php', 'getspell', $query, $db->mysql_error());
    $s = $db->mysql_fetch_array($result);
    return $s;
}
...
{
    global $db, $icon_cache, $icons_url;
