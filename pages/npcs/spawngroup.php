...
$result = db_mysql_query($query) or message_die('spawngroup.php', 'MYSQL_QUERY', $query, $db->mysql_error());
$spawn = $db->mysql_fetch_array($result);
...
if ($db->mysql_num_rows($result) > 0) {
    while ($row = $db->mysql_fetch_array($result)) {
        if ($sg != $row["sgid"]) {
            $sg = $row["sgid"];
            print "</ul><li><a href=?a=spawngroup&id=" . $row["sgid"] . ">" . $row["sgname"] . "</a>, range=";
