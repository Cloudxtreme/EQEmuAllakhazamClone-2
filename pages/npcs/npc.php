...
if ($id != "" && is_numeric($id)) {
    ...
    $QueryResult = db_mysql_query($Query) or message_die('npc.php', 'MYSQL_QUERY', $Query, $db->mysql_error());
    if ($db->mysql_num_rows($QueryResult) == 0) {
        header("Location: npcs.php");
        exit();
    }
    $npc = $db->mysql_fetch_array($QueryResult);
    $name = $npc["name"];
} elseif ($name != "") {
...
if ($use_custom_zone_list == TRUE) {
    ...
    if ($db->mysql_num_rows($result) > 0) {
        while ($row = $db->mysql_fetch_array($result)) {
...
if ($db->mysql_num_rows($result) > 0) {
    ...
    while ($row = $db->mysql_fetch_array($result)) {
        ...
        if ($display_spawn_group_info == TRUE) {
            $print_buffer .= "<li><a href='?a=spawngroup&id=" . $row["spawngroupID"] . "'>" . $row["spawngroup"] . "</a> : " . floor($row["y"]) . " / " . floor($row["x"]) . " / " . floor($row["z"]);
...
";
$result = db_mysql_query($query) or message_die('npc.php', 'MYSQL_QUERY', $query, $db->mysql_error());
if ($db->mysql_num_rows($result)) {
    $print_buffer .= "<h2 class='section_header'>Killing this NPC lowers factions with</h2><ul>";
    while ($row = $db->mysql_fetch_array($result)) {
       if ($row["id"] != 79 && $row["id"] != 143)  $print_buffer .= "<li><a href=?a=faction&id=" . $row["id"] . ">" . $row["name"] . "</a> (" . $row["value"] . ")";
    }
}
$print_buffer .= "</ul>";
$query = "
...
if ($db->mysql_num_rows($result)) {
    $print_buffer .= "
        ...
        <ul>";
    while ($row = $db->mysql_fetch_array($result)) {
        if ($row["id"] != 112) $print_buffer .= "<li><a href=?a=faction&id=" . $row["id"] . ">" . $row["name"] . "</a> (" . $row["value"] . ")";
    }
}
$print_buffer .= "</ul>";
$print_buffer .= "</td></tr></table>";
