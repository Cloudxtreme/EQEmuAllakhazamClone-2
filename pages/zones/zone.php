...
";
$result = db_mysql_query($query) or message_die('zones.php', 'MYSQL_QUERY', $query, $db->mysql_error());
$zone = $db->mysql_fetch_array($result);
...
if ($mode == "npcs") {
    ...
    $result = db_mysql_query($query) or message_die('zone.php', 'MYSQL_QUERY', $query, $db->mysql_error());
...
if ($mode == "items") {
    ...
    while ($row = $db->mysql_fetch_array($result)) {
        ...
        $result2 = db_mysql_query($query) or message_die('zone.php', 'MYSQL_QUERY', $query, $db->mysql_error());

        if ($db->mysql_num_rows($result2) > 0) {
            $ItemsFound = $db->mysql_num_rows($result2);
        }

        while ($res = $db->mysql_fetch_array($result2)) {
