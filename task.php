...
if ($id != "" && is_numeric($id) && $display_task_info == TRUE) {
    ...
    $QueryResult = db_mysql_query($Query) or message_die('task.php', 'MYSQL_QUERY', $Query, $db->mysql_error());
    if ($db->mysql_num_rows($QueryResult) == 0) {
        header("Location: customzoneslist.php");
        exit();
    }
    $task = $db->mysql_fetch_array($QueryResult);
    $name = $task["title"];
} else {
    header("Location: customzoneslist.php");
    exit();
...
if ($display_task_activities == TRUE) {
    ...
    if ($db->mysql_num_rows($QueryResult) > 0) {
        ...
        while ($row = $db->mysql_fetch_array($QueryResult)) {
            ...
            if ($GoalMethod == 1) {
                ...
                $QueryResult2 = db_mysql_query($Query2) or message_die('task.php', 'MYSQL_QUERY', $Query2, $db->mysql_error());
                $GoalListString = "";
                if ($db->mysql_num_rows($QueryResult2) > 0) {

                    while ($row2 = $db->mysql_fetch_array($QueryResult2)) {
